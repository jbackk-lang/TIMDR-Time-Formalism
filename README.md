...
bash — uruchom `pytest tests/ -v` przed zaufaniem tym liczbom, tak jak
w każdym poprzednim repo tej rodziny.
...
**⚠️ Nie uruchomione w tej sesji.** Testy w `tests/test_fourier_bridge.py`
używają szerokich tolerancji (±30%) dla części opartej na impulsie
gaussowskim — dyskretyzacja/skończone okno FFT odchyla wynik od
granicy ciągłej, a bez wykonania nie da się tego dokładnie oszacować.
Część jednotonowa (dokładny test na jednym binie FFT) nie ma tego
problemu — to jest algebra, nie przybliżenie. Uruchom
`pytest tests/test_fourier_bridge.py -v`.
